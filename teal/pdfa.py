    ...
    def validate_pdf(self, data, filename, profile='0') -> JSONResponse:
        ...
        if result.returncode == 0 or result.returncode == 1:
            ...
            with open(tmp_file_out_path) as tmp_json:
                ...
                out['profile'] = report['report']['jobs'][0]['validationResult']['profileName'].split(' ')[0]
                return create_json_response(content=jsonable_encoder(PdfAReport.model_validate(out)),
                                            background=BackgroundTask(cleanup_tmp_dir, tmp_dir))
