...
def test_non_zero_return_code():
    ...
    assert json.loads(resp.body) == {
        "message": "got return code 127 'test.pdf' /bin/sh: 1: foo: not found\n"
    }
...
def test_validate_pdfa_failed():
    validator = pdfa.PdfAValidator()
    resp = validator.validate_pdf(
        load_file("data/ocr/scanned_document_with_table.pdf"), filename="test.pdf"
    )
